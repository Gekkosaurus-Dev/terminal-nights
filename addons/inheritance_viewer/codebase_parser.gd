	...
	for cname in result:
		if not result[cname].get("builtin", false):
			...
			if source != "":
				...
				# ADDED: Pass _lang(path) to parser
				result[cname]["outbound_calls"] = _parse_outbound_calls(stripped, result, _lang(path))
	return result
...
static func _lang(path: String) -> String:
	...
	if ext == "gd":    return "gd"
	if ext == "cs":    return "cs"
	if ext in ["cpp", "cxx", "cc", "h", "hpp", "hxx"]: return "cpp"
	return ""
...
# ---------------------------------------------------------------------------
# Signal connections (GDScript only — C# uses events differently)
# ---------------------------------------------------------------------------
...
# ADDED: lang parameter and caller function tracking
static func _parse_outbound_calls(stripped: String, class_lookup: Dictionary, lang: String) -> Array:
	var calls = []
	var seen  = {}
	...
	for m in call_rx.search_all(stripped):
		...
		if class_lookup.has(receiver): target = receiver
		else:
