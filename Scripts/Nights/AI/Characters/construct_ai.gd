	...
	if allowed_to_move():
		match step:
			0:
				...
				print("construct phase 2")
			1:
				move_to(ROOM_04,State.ALT_2) #phase 3
				print("construct phase 3")
			2:
				move_to(ROOM_05,State.PRESENT) #phase 4
				state = Constr_states.PHASE_4
				print("construct phase 4 --> left for the hallway")
				#trigger attack after opening left hall cam OR after 25s passed
			3:
				move_to(ROOM_04,State.PRESENT,-step)
				print("construct returned to the bazaar")
				#go back to start

# TODO: after hitting door, take x% power and return to bazaar in phase 1 or 2
...
func allowed_to_move():
	...
	# after cams go off, remain locked for random 0.83-16.6s
		...
		if (state == Constr_states.LOCKED):
			...
			return false
		elif (state == Constr_states.PHASE_4):
			#print("already in the hallway")
			return false
	return true
